...
**Example integration tests** (requires live AWS + OCM credentials): to test a single example during development, use `scripts/run-example.sh <example_name>` (requires `RHCS_TOKEN` and `TF_VAR_cluster_name`). For **pre-release validation** of all Classic examples, use `scripts/run-all-examples.sh`, which runs `terraform apply` + `destroy` across the example suite. Use `--dry-run` to validate prerequisites, `--include`/`--exclude` to select examples, and `--cluster-prefix` to namespace clusters. Always set `--cluster-prefix` to something identifiable (for example, your name or ticket) so test clusters are easy to find and clean up. Requires `RHCS_TOKEN`; the shared-VPC example also needs credentials for the shared AWS account (see `scripts/run-all-examples.sh --help`). To run against a staging environment, set `RHCS_URL`.
